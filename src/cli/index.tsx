...
export default function CLIApp() {
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'assistant', content: 'Daedalus CLI ready. Commands: docs, verify, help' }
  ])
  ...
  const processCommand = async (command: string, docs: Document[]): Promise<string> => {
  ...
  const formatDocuments = (docs: Document[]): string => {
    ...
    return docs.map(doc => {
      const statusColor = doc.status === 'completed' ? 'green' :
        doc.status === 'verified' ? 'green' :
        doc.status === 'in-progress' ? 'blue' : 'yellow'
...
`
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="blue" padding={1}>
      <Text bold color="blue">Daedalus - Agent Workflow System</Text>
